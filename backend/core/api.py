...
# from .models import UserTourPreference
# from .recommendations import recommend_tours  # Assuming recommend_tours function is correctly implemented
# @api_view(['POST'])
...
#         preferences = UserTourPreference.objects.get(user=request.user)

#         # Call the recommend_tours function to get tour recommendations
#         results = recommend_tours(preferences)

#         # Return the list of recommended tours as a response
#         return Response({
#             'recommendations': [
#                 {
#                     'id': r['tour'].id,
#                     'title': r['tour'].title,
#                     'price': r['tour'].price_per_person,
#                     'match_score': r['match_percentage'],
#                     'image': request.build_absolute_uri(r['tour'].main_image.url) if r['tour'].main_image else None
#                 }
#                 for r in results
#             ]
#         })
#     except UserTourPreference.DoesNotExist:
#         return Response({"error": "User preferences not set"}, status=400)
#     except Exception as e:
